...
private:
    ...
    std::optional<RowId> create_initial_tree(IndexKey key, const RowId& row_id);

    // Traverse from the root to the leaf where this key belongs.
    std::optional<std::uint32_t> find_leaf_page_id(IndexKey key);

    // Insert directly into a leaf that still has space.
    std::optional<RowId> insert_into_leaf(std::uint32_t leaf_page_id, IndexKey key, const RowId& row_id);

    // Split a full leaf, then push the separator into its parent.
    std::optional<RowId> split_leaf_and_insert(std::uint32_t leaf_page_id, IndexKey key, const RowId& row_id);
